...
When another player loots an item that matches the active character's tracked wishlist, the addon SHALL show an alert popup identifying the looting player and the item. The addon SHALL NOT change the active character's possession indicator or remembered best looted item level based on another player's loot.
...
- **THEN** the addon shows an alert popup naming the player and item
