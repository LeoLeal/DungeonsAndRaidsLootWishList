...
- Add tracker tag filtering that only offers tags currently used by at least one item, supports multi-select OR filtering, treats no selection as show-all, and automatically prunes stale selections when tags are deleted or become unused.
- Keep tracker filter state presentation-only so it never changes tracked membership and never suppresses loot alerts or loot roll badges.
...
- `tracker-tag-filtering`: Tracker tag filter controls, presentation-only multi-select OR filtering, stale selection pruning, and tag-aware tooltip footer presentation.
