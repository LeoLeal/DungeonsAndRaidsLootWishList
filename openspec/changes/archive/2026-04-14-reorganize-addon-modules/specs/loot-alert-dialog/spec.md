...
#### Scenario: Alert dialog consumes normalized alert records
- **WHEN** the alert dialog is shown for a tracked-loot event
- **THEN** `LootAwareness/Alerts` modules drive the dialog from a normalized alert record
- **AND** bootstrap code does not replay raw event payloads to reconstruct the alert UI
